...
def new_codraft():
    if request.method == "POST":
        ...
        new_codraft = CoDraft(title=title, category=category, creator_id=session["user_id"], max_units=maxcontribution, max_unit_type=unittype)
        new_contribution = Contribution(codraft=new_codraft, author=User.query.get(session["user_id"]), text_snapshot=text)
